...
func (pc *Controller) sync(ctx context.Context, key string) error {
	...
	// Sanity check.
	if pod.Spec.NodeName != pc.nodeName {
		klog.V(2).InfoS("No relationship found between Pod and Node", "Pod", klog.KObj(pod), "Node", pc.nodeName)
		return nil
	}
	...
	for _, v := range pod.Spec.Volumes {
		...
		err = pc.syncVolume(ctx, key, spec)
		if err != nil {
