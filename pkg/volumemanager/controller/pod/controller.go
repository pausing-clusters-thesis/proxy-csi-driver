...
import (
	...
	delayedcsi "github.com/pausing-clusters-thesis/proxy-csi-driver/pkg/volume/csi"
...
type Controller struct {
	...
	driverRegistry                    *delayedcsi.DriverRegistry
}
...
func NewController(
	...
	pc := &Controller{
		...
		targetPathBindManager:             targetPathBindManager,
...
func (pc *Controller) enqueuePodsThroughPersistentVolumeClaim(depth int, obj kubeinterfaces.ObjectInterface, op socontrollerhelpers.HandlerOperationType) {
	...
	klog.V(5).InfoS("Controller.enqueuePodsThroughPersistentVolumeClaim", "PersistentVolumeClaim", klog.KObj(pvc))

	if !controllerhelpers.HasAnnotation(pvc, naming.DelayedStorageBackendPersistentVolumeClaimRefAnnotation) {
		// PVC is not a proxy PVC, but it can still be backing, so we check for a proxy ref.
	...
	klog.V(5).InfoS("Controller.enqueuePodsThroughPersistentVolumeClaim: enqueuing all controller objects referencing claim", "PersistentVolumeClaim", klog.KObj(pvc))
...
func (pc *Controller) enqueuePodsThroughPersistentVolume(depth int, obj kubeinterfaces.ObjectInterface, op socontrollerhelpers.HandlerOperationType) {
	...
	klog.V(5).InfoS("Controller.enqueuePodsThroughPersistentVolume", "PersistentVolume", klog.KObj(pv))
...
func (pc *Controller) enqueuePodsThroughVolumeAttachment(depth int, obj kubeinterfaces.ObjectInterface, op socontrollerhelpers.HandlerOperationType) {
	...
	klog.V(5).InfoS("Controller.enqueuePodsThroughVolumeAttachment", "VolumeAttachment", klog.KObj(va))

	if va.Spec.NodeName != pc.nodeName {
		return
	}
